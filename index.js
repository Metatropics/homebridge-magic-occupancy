...
module.exports = function (homebridge) {
    ...
    Characteristic.KeepingOccupancyTriggered.UUID =
        '25eb64e4-e104-11eb-ba80-0242ac130004'
...
class MagicOccupancy {
    constructor (log, config) {
        ...
        //Handle restoring state - gotta restart the decaying timer if we rebooted
            ...
            this.persistBetweenReboots &&
            savedState.ModeState == 'UnoccupiedDelay'
        ) {
    ...
    _getInitialMainCacheState () {
        ...
        try {
            ...
            if (!('ModeState' in state)) {
                state.ModeState =
                    defaultState._last_occupied_state == true
                        ? 'UnoccupiedDelay'
                        : 'Unoccupied'
            }
            ...
            this.log.debug(
                `Error initializing past state, falling back on default - ${error}`
            )
            return defaultState
        }
    }
    ...
    checkOccupancy (timeoutUntilCheck = 0) {
        ...
        this.log.debug(
            `checking occupancy. Total: ${switchesToCheck.length} switches`
        )
...
class StatefulSwitch extends BaseHelperSwitch {
    ...
    _handleNewState (on) {
        const isValidOn =
            on &&
            (!this.stayOnOnly || this.occupancySensor._last_occupied_state == true)
        ...
        this.occupancySensor.occupancyService
            ...
            .on(
                ...
                function (occVal, callback) {
                    ...
                    this._service.getCharacteristic(Characteristic.On).getValue(
                        function (err, value) {
                            if (err) {
                                this.log(`ERROR GETTING VALUE Characteristic.On ${err}`)
                                return
                            }
                            ...
                            if (value != this.occupancySensor._last_occupied_state) {
                                this._service.setCharacteristic(
                                    Characteristic.On,
                                    this.occupancySensor._last_occupied_state
                                )
                            }
                        }.bind(this)
                    )
